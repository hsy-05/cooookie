...
![後台最新消息列表截圖](z_demo-images/admin/news_list.png)
...
![客服回覆管理](z_demo-images/admin/contact_system.png)
...
![後台操作紀錄畫面](z_demo-images/admin/action_logs.png)
...
![圖片上傳組件範例](z_demo-images/admin/image_uploader.png)
...
- **首頁佈局**: 整合 Banner 輪播、精選產品與最新消息，採分區明確的區塊化設計。採用溫慢的手作餅乾品牌主色，jQuery 動態驅動 Banner 輪播切換，點擊卡片平滑導向詳情頁。
...
- **產品詳情**: 採左圖右文的排版。左側產品圖支援 jQuery 點擊互動放大與多圖切換，右側重點商品規格加粗顯示。
  ...
  <p align='center'><img src='z_demo-images/frontend/home.png' width='600'></p>
...
本網站後台採用嚴謹的「角色權限控制 (Role-Based Access Control)」，並實作「視覺隱藏 (AdminLTE)」與「後端核心攔截 (Middleware)」雙重安全防線。

### 📁 核心檔案權限職責對照表

| 檔案路徑 | 技術角色 | 白話核心用途說明 (做什麼事) |
| :--- | :--- | :--- |
| `config/backend_permissions.php` | 權限字典檔 | 全站點記法（如 `news.view`）的合法權限清單，系統防呆的一體化字典。 |
| `app/Helpers/PermissionHelper.php` | 格式化工具 | 清洗與解析字典檔，提供全站靜態快取（Static Cache），優化主機記憶體效能。 |
| `app/Models/Admin.php` | 護照驗證中心 | 提供 `$admin->canDo($permission)` 核心方法，判斷目前登入者的身份與權限階層。 |
| `app/Http/Middleware/CheckBackendPermission.php` | 路由層保全 | 阻擋惡意「直接輸入網址」的行為，未授權者會自動觸發防呆並被安全退回前一頁。 |
| `config/adminlte.php` | 前端視覺遮罩 | 配合 Laravel 權限機制，若管理員無權限，左側功能選單會貼心自動隱藏。 |

---

### 🚀 新增或修改權限的三步驟標準流程 (SOP)

當未來網站要擴充「購物車」或「會員管理」功能，需要限制某個頁面只有特定人能看時，請務必嚴格執行以下三個步驟：

#### 步驟一：在字典檔中註冊新權限 (`config/backend_permissions.php`)
```
'shop_management' => [
    'label' => '購物車管理',
    'subs' => [
...
[ 使用者發起 Request 請求 ]
          ...
          v
================================================================================
【第一關：路由與保全攔截】 routes/web.php & CheckBackendPermission.php
================================================================================
          |
          +---> 檢查路由有沒有設定 `admin.perm:system.view` 中間件？
                      |
                      +---> [沒有設定] ---> 直接放行進入 Controller。
                      |
                      +---> [有設定]   ---> 保全啟動，抓取當前登入的 Admin 物件。
                                                |
                                                v
                                  呼叫呼叫 Admin.php 裡面的 `$admin->canDo('system.view')`
                                                |
                                                v
================================================================================
【第二關：護照與核心邏輯判斷】 app/Models/Admin.php -> canDo()
================================================================================
                                                |
                                                v
       [防呆比對] 呼叫 `PermissionHelper::getValidPermissions()` 讀取 `backend_permissions.php`
                      |
                      +---> 這張門票合法嗎？
                                 |
                                 +---> [不合法] ---> 門口直接攔截，回傳 false。
                      ...
                      +---> 身份是【最高開發者 (isDeveloper)】嗎？
                                 ...
                                 +---> (否) ───> 繼續檢查一般管理員邏輯。
                                             |
                                             v
                      +---> 檢查這張票是 `system.` 開頭的（開發者專用）嗎？
                                 |
                                 +---> (是) ───> 你不是開發者，卻想看開發者頁面！回傳 false。
                                 |
                                 +---> (否) ───> 檢查他個人的 checkbox 有沒有勾選此權限。
                                             |
                                             v
================================================================================
【第三關：保全處置】 Middleware 根據 canDo() 的結果放行或轟走
================================================================================
                                             |
                      +----------------------+
                      |
             ...
             canDo() 回傳什麼結果？
                      |
                      +---> [ 回傳 true ]  ──> 安全通關，執行 `return $next($request)` 進入頁面。
                      |
                      +---> [ 回傳 false ] ──> 呼叫 `BaseAdminController::showMsg()` 存入錯誤訊息，
                                               並用 `redirect($url)->withInput()` 轟回前一頁！
  </pre>
